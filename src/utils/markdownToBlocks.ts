...
import {arrayFieldSchemaFactory, blockFieldSchemaFactory, imageFieldSchemaFactory} from '@/helpers/sanity/fieldSchemaFactories'
...
const mockSchema = Schema.compile({
  ...
  types: [{
    ...
    type: 'object',
    fields: [
      arrayFieldSchemaFactory('body').name('body').of([
        blockFieldSchemaFactory('block').build(),
        imageFieldSchemaFactory('image').build(),
      ]).build(),
    ],
  }],
})

const blockContentType = mockSchema.get('mock').fields.find((field: any) => (
  field.name === 'body'
)).type
