...
function generateKey(length = 8) {
  return crypto.randomBytes(length * 2).toString("base64").replace(/[^\da-z]/gi, "").slice(0, length);
}
  ...
  const buildFn = (keys) => {
    ...
    get(target, prop) {
      if (prop !== "build" && prop !== "anonymous" && typeof prop == "string")
        return (value) => {
          if (typeof value > "u")
            clonedSchema = omit(clonedSchema, [prop]);
          else {
...
function markdownToBlocks(input) {
  const html = md({ html: !0 }).render(input);
...
function contentfulEntryToSanityObject(id, entry, locale, data, options) {
  ...
  Field: ${key}`
      ...
      Array.isArray(value) && (doc[key] = compact(
...
function localeDataFromExport(data, opts) {
  ...
  const localesToImport = (useMultiLocale ? (data.locales ?? []).map(({ code }) => code) : opts.locale ? [opts.locale] : compact([defaultLocale?.code])).filter((code) => data?.locales?.some((locale) => locale.code === code));
  if (!defaultLocale)
    throw new ContentfulNoDefaultLocaleError();
...
async function contentfulToDataset(exports$1, opts) {
  ...
  invariant(exports$1.drafts.entries, "Expected data.entries to be defined"), invariant(exports$1.drafts.entries.length > 0, "Expected data.entries to be defined");
...
async function convertUnsupportedImages(dataset, exportDir, contentTypeLookup) {
  ...
  const rl = readline.createInterface({
    input: stream,
    crlfDelay: 1 / 0
  }), assetsDir = path.join(exportDir, "assets");
    ...
    try {
      ...
      for (; (match = regex.exec(line)) !== null; ) {
        ...
        if (contentTypeLookup(url) === "image/avif") {
          const fileName = path.parse(url).name, filePath = path.join(exportDir, "assets", `${fileName}.png`);
          await fetch(url).then((res) => res.buffer()).then((buffer) => sharp(buffer).png().toFile(filePath)).then(() => newLine = newLine.replace(url, `file://${filePath}`)).catch((err) => {
            console.error(err);
          });
        }
      }
...
async function optimizeSVG(dataset, exportDir) {
  ...
  for await (const line of rl) {
    ...
    try {
      ...
      let match;
      for (; (match = regex.exec(line)) !== null; ) {
        const svgUrl = match[1], response = await fetch(svgUrl);
        if (response.status === 200) {
          const svgArrayBuffer = await response.arrayBuffer(), svgBuffer = Buffer.from(svgArrayBuffer), svgFileName = svgUrl.substring(svgUrl.lastIndexOf("/") + 1), svgFilePath = path.join(exportDir, "assets", svgFileName);
          fs.writeFileSync(svgFilePath, svgBuffer);
          const svgFileContents = fs.readFileSync(svgFilePath, {
            encoding: "utf8"
          }), { data: optimizedSVG } = await optimize(svgFileContents);
          fs.writeFileSync(svgFilePath, optimizedSVG), newLine = newLine.replace(svgUrl, `file://${JSON.stringify(svgFilePath).slice(1, -1)}`);
        } else
...
async function datasetAction({
  ...
  ), datasetFilePath = path.join(exportDir, datasetFile);
  invariant(
    isAbsolutePath(datasetFilePath),
    `datasetFilePath must be an absolute path: ${datasetFilePath}`
  );
  const draftData = JSON.parse(await readFile(exportFilePath, "utf8")), publishedData = JSON.parse(
...
async function exportAction({
  ...
  const exportDir = absolutify(_exportDir);
  invariant(isAbsolutePath(exportDir), "exportDir must be an absolute path"), await mkdirp(exportDir);
  const options = {
    spaceId,
    managementToken,
    environmentId,
  ...
  await contentfulExport(options), await contentfulExport({
    ...options,
    ...
    contentFile: path$1.parse(exportFile).name + ".published.json"
  });
}
...
` + new Error().stack
    ...
    function loadLocale(name) {
      ...
      if (locales[name] === void 0 && module && module.exports && isLocaleNameSane(name))
        try {
...
function contentfulTypeToSanitySchema(contentType, data, flags) {
  ...
  return schemaType.fields = compact(
...
async function contentfulToStudioSchema(data, opts) {
  ...
  const allSchemaTypes = schemas.map((schema) => {
    const identifier = `${Case.camel(schema.name)}Type`, stringifiedDefinition = stringify(schema, (value, space, next, key) => key === "validation" ? Array.isArray(value) && value.length > 0 ? `Rule => Rule.${value.map((r) => serializeRuleSpecToCode(r)).join(".")}` : void 0 : opts.typescript && key === "fields" ? Array.isArray(value) && value.length > 0 ? `[${value.map((v, k) => `defineField(${next(v, k)})`).join(",")}]` : void 0 : next(key === "initialValue" && !useMultiLocale ? value[defaultLocale.code] : value, key)), definition = `
    export const ${schema.name}Type = ${opts.typescript ? `defineType(${stringifiedDefinition})` : stringifiedDefinition};`;
...
async function schemaAction({
  ...
  const typescript = schemaFile.endsWith(".ts"), data = JSON.parse(await readFile(exportFilePath, "utf8")), schemaTypes = await contentfulToStudioSchema(data, {
    typescript,
    intlMode,
    keepMarkdown,
...
const datasetActionArgs = z.object({
...
function formatError(err) {
  return isValidationErrorLike(err) ? err : err instanceof z.ZodError ? fromZodError(err) : err instanceof Error ? err : new Error("Unknown error", { cause: err });
}
export {
  batchAction,
  batchActionArgs,
  contentfulToDataset,
  contentfulToStudioSchema,
  datasetAction,
  datasetActionArgs,
  exportAction,
  exportActionArgs,
